...
class AlgoliaIndex {
  ...
  async indexVideoChunks(videoID, transcript) {
    try {
      const records = transcript.utterances.filter((u) => u.text.length >= MIN_CHUNK_LENGTH).map((u) => {
        const start = u.start;
        const end = u.end;
        const text = u.text.trim();
        const summary = transcript.summary
        return { objectID: `${videoID}-${start}`, videoID, start, end, text, summary: summary };
      });
      const res = await this.algolia.saveObjects({indexName: this.index,objects: records, });
